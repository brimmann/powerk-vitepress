...
## What is Python?
As I mentioned earlier Python is something that we use to write(build) computer programs with. It is refered to as a programming language. Before we do anything I want you to take a look a your computer and see how many kinds of programs we have. We have programs like notepad and also have websites which are type of programs that run in your browser. Similary there are programs that can only be used with a keyboard that don't have a graphical user interface. While they arn't as popular nodawys but they were quite common at the begining of computer era. Please go ahead and search for '*What are CLI programs?*' and take a look. These CLI programs are still popular among develpers and you will learn about them soon as we move forward. There are so many other programming languages out there but we do Python in this crash course.
...
### What do we need?
**Assumption**: I asume that you are a Windows user since it is quite common among computer owners. You can follow the whole course even if you don't have a Windows machine but, you will need to figure out how to setup the same environmetn on your system. And any Python program you write will run on all modern computers regradless of their operating system.

To start writing our first program you need to install the following two things in your computer:
- Python
- VS Code

Once you have installed these two things, you will be able to write any pogram and run it on your computer.

### Installing Python
Installing Python is quite sraight forward. Click on [this link](https://www.python.org/downloads/). Use the **Download Python** button that appears first on the page to download the latest version.
...
**Note**: Just follow along and don't worry if it doesn't make sence yet.
...
After going through the steps above, you will see that your foler is opened in VS Code. Now we can create our first python code inside this folder using VS Code.
...
2. You will see a few icons apear to the right of your folder's name. Click on the first icon in the row which apears to be a document with a plus icon.
...
2. You will see something apeared at the bottom of your VS Code that allows you to type something in. This is called the Terminal where we can internact with our computer using a CLI(Command Line Interface). Remember I asked to go and search for *'What are CLI programs?'*. Make sure you have done it. No click somewhere in the Terminal.
3. Type this ```python hello.py``` in the Terminal and press enter.

Now you will see that that *'Hello world!'* text from our code we wrote in the previous section apears below the command you just entered.
...
### What happened?
You might be expecting our program to be something similar to Notepad, a website or something which has window-like struture that you can insall or open from your desktop. But our program is just a text *Hello world* that apeared in some part of our VS Code, it wasn't somehting like Notepad or a webiste. You are right. However, programs are necessarily Notepad or a website, they are more then that. In the begining of computer era we didn't have programs like Notpad or websites. They were most like the Terminal in our VS Code, where all you could see was just text on the black background and you would type things in them jsut like the command ```python hello.py``` we typed. 

The good news is that all prorams are built the same way. You can create a program like notepad or a website the same way we creted our hello world program. They are all written the same way and follow the same principle regardless of whether they are programs like Notepad, webites or just a text-based old-fashined CLI program.

So we created CLI program which print a text 'Hello world!' to the Terminal.

We can create advanced programs like Notepad or a website using Python. However we need to learn the universal principle of prgramming and how they are written by developing text-only-based programs that run in the Terminal. Once we have learned the basics we can move on to build the programs of our choice. After you completed this course, we have a dedicated course for those advanced Python programs, [Python Projects: Building Cool Stuff](/python-stuff/), where you can continue your journy.
...
Make sure you have done everything properly and understand most of this chapter. Let's dive deeper. Welcome to the world of prgramming.